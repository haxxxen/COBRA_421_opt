...
int ps3mapi_set_process_mem(process_id_t pid, uint64_t addr, char *buf, int size)
{
	...
	else return process_write_memory(process, (void *)addr, (void *)get_secure_user_ptr(buf), size, 1);
}
...
int ps3mapi_get_process_mem(process_id_t pid, uint64_t addr, char *buf, int size)
{
	...
	int ret = process_read_memory(process, (void *)addr, (void *)get_secure_user_ptr(buff), size);
...
int ps3mapi_process_page_allocate(process_id_t pid, uint64_t size, uint64_t page_size, uint64_t flags, uint64_t is_executable, uint64_t *page_address)
{
	...
	if (ret != SUCCEEDED) { return ENOMEM; }
	if (is_executable == 0)
	{
	...
	if (kbuf) { page_free(process, kbuf, flags); }
	return SUCCEEDED;
}
...
int ps3mapi_load_process_modules(process_id_t pid, char *path, void *arg, uint32_t arg_size)
{
	...
	// if(strstr(path, "dev_flash")) ret = prx_start_module(prx, process, 0, 0);
	// else ret = prx_start_module_with_thread(prx, process, 0, (uint64_t)vbuf);
	// ret = prx_start_module(prx, process, 0, &vbuf);
	...
	if (ret != SUCCEEDED)
	{
		// if(strstr(path, "dev_flash")) prx_stop_module(prx, process, 0, 0);
		// else prx_stop_module_with_thread(prx, process, 0, 0);
		// prx_stop_module(prx, process, 0, 0);
...
int ps3mapi_unload_process_modules(process_id_t pid, sys_prx_id_t prx_id)
{
	...
	// int ret = prx_stop_module(prx_id, process, 0, 0);
...
int ps3mapi_get_process_module_info(process_id_t pid, sys_prx_id_t prx_id, sys_prx_module_info_t *info)
{
	...
	if (ret != SUCCEEDED) return EINVAL;
	if ((modinfo.segments == 0) || (modinfo.filename == 0) || (modinfo.segments_num == 0) || (modinfo.filename_size == 0)) { return EFAULT; }
