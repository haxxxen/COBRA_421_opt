...
void abort(void)
{
	_debug_printf("abort() called! LV1 Panic\n");
	lv1_panic(0);
	while (1);
}
...
/* int logprintf(const char *fmt, ...)
{
	...
	if (vars->debug_offset + written <= MAX_LOG)
	{
		...
		fd = cellFsOpen(DUMP_FILE, CELL_FS_O_WRONLY|CELL_FS_O_CREAT|CELL_FS_O_TRUNC, &fd, 0666, NULL, 0);
		if (fd >= 0)
		{
			// ufs_write(fd, vars->debug_offset, strBuf, written);
			// ufs_close(fd);
			cellFsWrite(fd, vars->debug_offset, strBuf, written);
			cellFsClose(fd);
