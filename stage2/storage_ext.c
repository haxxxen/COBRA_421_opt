...
//
// uint8_t condition_toggle_psxemu=0;
...
static INLINE void get_next_read(int64_t discoffset, uint64_t bufsize, uint64_t *fileoffset, uint64_t *readsize, int *file)
{
	...
	*fileoffset = 0;
	...
	DPRINTF("COBRA :::: Offset or size out of range  %lx   %lx!!!!!!!!\n", discoffset, bufsize);	
}
	...
	while (remaining > 0)
	{
		...
		{		
			if (discfd == -1 || file != activefile)
			{
				...
				DPRINTF("COBRA :::: Changed to part file %d\n", file);
...
int process_proxy_cmd(uint64_t command, process_t process, uint8_t *buf, uint64_t offset, uint32_t size)
{
	...
	if (!do_copy)
	{
		...
		if (ret != 0)
		{
			DPRINTF("COBRA :::: event_queue_receive failed: %x\n", ret);
			return ret;
		}
	...
	else
	{
		...
		if (ret != 0)
		{
			DPRINTF("COBRA :::: page_allocate failed: %x\n", ret);
			return ret;
		}
		...
		if (ret != 0)
		{
			DPRINTF("COBRA :::: page_export_to_proc failed: %x\n", ret);
	...
	if (ret != 0)
	{
		DPRINTF("COBRA :::: proxy read failed: %x\n", ret);
	}
...
int process_get_psx_video_mode(void)
{
	...
	if (effective_disctype == DEVICE_TYPE_PSX_CD)
	{
		...
		if(ret == 0) DPRINTF("COBRA :::: NTSC\n");
		if(ret == 1) DPRINTF("COBRA :::: PAL\n");
...
void dispatch_thread_entry(uint64_t arg)
{
	...
	//DPRINTF("COBRA :::: Exiting dispatch thread %d\n", ret);
	ppu_thread_exit(0);
}
...
void process_disc_insert(uint32_t disctype)
{
	...
	fake_disctype = 0;
	DPRINTF("COBRA :::: real disc type = %x\n", real_disctype);
...
LV2_HOOKED_FUNCTION_COND_POSTCALL_8(int, emu_read_bdvd0, (void *object, uint64_t offset, void *buf, uint64_t size, int r7, uint64_t r8, uint64_t r9, uint64_t r10, uint64_t st0, uint64_t st1))
{
	...
	{
#ifdef DEBUG
		DPRINTF("COBRA :::: Warning: emu_read_bdvd0 called.\n");
		dump_stack_trace2(16);

		if (r7 != 1 || r8 != 0 || r9 != 0 || r10 != 0 || st0 != 0 || st1 != 1)
		{
			DPRINTF("COBRA :::: emu_read_bdvd called with unknown params\n");
			//dump_stack_trace2(16);
			fatal("aborting.\n");
		}
#endif			
...
LV2_HOOKED_FUNCTION_PRECALL_SUCCESS_2(int, post_storage_get_device_info, (uint64_t device_id, device_info_t *device_info))
{
	if (device_id == BDVD_DRIVE)
	{
		...
		if (effective_disctype && disc_emulation)
		{
			...
			DPRINTF("COBRA :::: Faked size to %lx\n", device_info->sector_count);
		}
	...
	if (open_shared_kernel_object(table, device_handle, (void **)&sys_storage_object, &obj_handle, SYS_STORAGE_HANDLE_OBJECT, 1) == 0)
	{
		...
		if (device == BDVD_DRIVE)
		{
			...
			{			
				...
				if (unk2 != 0)
				{
					DPRINTF("COBRA :::: WARNING: unk2 not 0: %lx\n", unk2);
				}
...
int process_generic_iso_scsi_cmd(uint8_t *indata, uint64_t inlen, uint8_t *outdata, uint64_t outlen)
{
	...
	switch (indata[0])
	{
		case SCSI_CMD_GET_EVENT_STATUS_NOTIFICATION:
		{
			...
			{
				//DPRINTF("COBRA :::: Event status: %02X\n", cmd->notification_class_request);
			}
		}			
		break;
		...
		/*default:
			DPRINTF("COBRA :::: Command %s outlen=%ld\n", get_scsi_cmd_name(indata[0]), outlen); */
	}
...
int process_cd_iso_scsi_cmd(uint8_t *indata, uint64_t inlen, uint8_t *outdata, uint64_t outlen, int is2048)
{
	...
	switch (indata[0])
	{
		case SCSI_CMD_READ_TOC_PMA_ATIP:
		{
			...
			{
				DPRINTF("COBRA :::: Requesting something other than TOC: %d!!\nPassing command to real function.", GET_FORMAT(cmd));
				return 0;
			}

			if (GET_MSF(cmd))
			{
				DPRINTF("COBRA :::: Warning: requesting tracks in MSF format. Not implemented.\n");
				return -1;
			}
			...
			if (outlen == 12)
			{
				...
				{
					DPRINTF("COBRA :::: Track out of range %d\n", cmd->track_session_num);
				}
		...
		case SCSI_CMD_READ_TRACK_INFORMATION:
		{
			...
			if (is2048)
			{
				DPRINTF("COBRA :::: READ TRACK INFORMATION not implemented for 2048 cd iso!\n");
				return -1;
			}

			if (cmd->rv_o_type != 1)
			{
				DPRINTF("COBRA :::: rv_o_type = %x, not implemented\n", cmd->rv_o_type);
				return -1;
			}
		...
		case SCSI_CMD_READ_CD:
		{
			...
			{
				DPRINTF("COBRA :::: Unexpected value for misc: %02X\n", cmd->misc);
				return -1;
			}

			if (cmd->rv_scsb != 0 && cmd->rv_scsb != 2)
			{
				DPRINTF("COBRA :::: Unexpected value for subchannel: %02X\n", cmd->rv_scsb);
				return -1;
			}

			if (GET_EXPECTED_SECTOR_TYPE(cmd) != 0)
			{
				DPRINTF("COBRA :::: Unexpected value for expected sector type: %d\n", GET_EXPECTED_SECTOR_TYPE(cmd));
				return -1;
			}
			...
			if (is2048)
			{
				DPRINTF("COBRA :::: Read CD on 2048 iso (lba=0x%x, length=0x%x)!!! Not implemented.\n", lba, length);
			...
			if (outsize == 0)
			{
				DPRINTF("COBRA :::: Warning: outsize is zero\n");
			}
			...
			return 1;			
			/*DPRINTF("COBRA :::: READ CD, sector %x size %x, expected sector type: %d\n", cmd->lba, s, GET_EXPECTED_SECTOR_TYPE(cmd));
			DPRINTF("COBRA :::: Misc: %02X, rv_scsb: %02X, outlen = %lu\n", cmd->misc, cmd->rv_scsb, outlen); */
...
{				
	...
	if (p == vsh_process)
	{
		...
		if (patch != 0)
		{
			...
			{
				case VSH_HASH:
					DPRINTF("COBRA :::: Patching Video Mode into spoofed DEBUG VSH..\n"); 
				...
				case VSH_DECR_HASH:
					DPRINTF("COBRA :::: Patching Video Mode into spoofed TOOL VSH..\n"); 
				...
				case VSH_NRM_HASH:
					DPRINTF("COBRA :::: Patching Video Mode into normal DEBUG VSH..\n"); 
				...
				case VSH_CEX_HASH:
					DPRINTF("COBRA :::: Patching Video Mode into spoofed RETAIL VSH..\n"); 
...
/* 	if (cellFsStat(PSX_FLAG, &stat) != 0)
	{
		if(cellFsOpen(PSX_FLAG, CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_TRUNC, &dst, 0666, NULL, 0) == CELL_OK)
		{
			cellFsWrite(dst, &size, 4, &size);
			cellFsClose(dst);
		}
		else
			DPRINTF("COBRA :::: Sorry, cannot write 'PSX-FLAG'\n");
	} */
	// cellFsOpen(PSX_FLAG, CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_TRUNC, &dst, 0666, NULL, 0);
	// cellFsWrite(dst, &size, 4, &size);
	// cellFsClose(dst);
/* 	else if (cellFsStat(PSX_FLAG, &stat) != 0)
	{
		if(cellFsOpen(NET_FLAG, CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_TRUNC, &dst, 0666, NULL, 0) == CELL_OK)
		{
			cellFsWrite(dst, &size, 4, &size);
			cellFsClose(dst);
		}
		else
			DPRINTF("COBRA :::: Sorry, cannot not write 'NET-FLAG'\n");
	} */
	// cellFsOpen(NET_FLAG, CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_TRUNC, &dst, 0666, NULL, 0);
	// cellFsWrite(dst, &size, 4, &size);
	// cellFsClose(dst);
	// condition_psx_psxemu = 1;
	// condition_net_psxemu = 1;
	// toggle = 1;
	condition_psxemu = 1;
}
...
static INLINE unsigned int get_disc_type(/*int hooked*/)
{
	...
	if (storage_open(BDVD_DRIVE, 0, &handle, 0) == 0)
	{
		...
		/*if (!hooked)
		{
			res = storage_send_device_command(handle, STORAGE_COMMAND_GET_DEVICE_TYPE, &indata, sizeof(indata), &disctype, sizeof(disctype), NULL);
		}
		else*/
		{
			res = (int)call_hooked_function_7(emu_storage_send_device_command, (uint64_t)handle, STORAGE_COMMAND_GET_DEVICE_TYPE, (uint64_t)&indata, sizeof(indata), (uint64_t)&disctype, sizeof(disctype), (uint64_t)NULL);
		}
...
/* static INLINE u64 lv2_peekd(u64 addr)
{
	u64 val;
	
	sys_cfw_peek(addr, &val);
	
	return val;
	// return *(u64 *)(HV_BASE+HV_SIZE+addr);
} */
/* static INLINE int mount_usb0(void)
{   
	int ret, n;
	char * usb_buf[64]; // 0xA is not sufficient!

	// try mount dev_usb000
	for(n = 0; n < 5; n++) {
		*usb_buf = alloc(sizeof(usb_buf), 0x27);
		// ret = sys_fs_mount_ext("CELL_FS_IOS:USB_MASS_STORAGE000", "CELL_FS_FAT", "/dev_usb000", 0, usb_buf, 0);
		ret = cellFsUtilMount("CELL_FS_IOS:USB_MASS_STORAGE000", "CELL_FS_FAT", "/dev_usb000", 0, 0, 0, usb_buf, 0);
		if(ret == EIO) {
			// DPRINTF("COBRA :::: Failed...Let's try again...\n");
			DPRINTF("COBRA :::: Failed to mount usb\n");
			timer_usleep(SECONDS(3));
		}
		else
			DPRINTF("COBRA :::: cellFsUtilMount: /dev_usb000\n");
		break;
    }
    return ret;
} */
...
{
	DPRINTF("COBRA :::: cellFsUtilMount: %s\n", mount_point);

/* 	if (!hdd0_mounted && strcmp(mount_point, "/dev_usb000") != 0 && strcmp(filesystem, "CELL_FS_FAT") != 0)
		mount_usb0();
	if (cellFsStat("/dev_usb000/nostage", &stat) == 0) {
		cellFsRename(CB_LOCATION_CEX, CB_LOCATION_CEX".bak");
		cellFsRename(CB_LOCATION_DEX, CB_LOCATION_DEX".bak");
		cellFsRename(SA_LOCATION, SP_LOCATION);
		cellFsRename(SM_LOCATION, SA_LOCATION);
		fatal("::: 'Safety-FLAG' detected...Let's shutdown...Bye Bye\n");
	}
	else */
	if (!hdd0_mounted && strcmp(mount_point, "/dev_hdd0") == 0 && strcmp(filesystem, "CELL_FS_UFS") == 0)
	{
		...
		// else if (cellFsStat(NET_FLAG, &stat) != 0)
		{
			cellFsOpen(NET_FLAG, CELL_FS_O_WRONLY | CELL_FS_O_CREAT | CELL_FS_O_TRUNC, &dst, 0666, NULL, 0);
			cellFsWrite(dst, &size, 4, &size);
			cellFsClose(dst);
		...
		if (!up_mode){
			on = 0;
			if (map_path("/app_home", "/dev_flash/rebug/packages", 0) == 0) on = 1;
			if (on){
				on = 0;
				if (map_path("/dev_flash/vsh/module/avc_plugin.sprx", "/dev_flash/vsh/resource/explore/icon/sman.xmb", 0) == 0) on = 1;
				if (on){
					on = 0;
					if (map_path("/dev_flash/vsh/module/edy_plugin.sprx", "/dev_hdd0/game/XMBM00001/USRDIR/edy_plugin.sprx", 0) == 0)  on = 1;
				}
			}
		}
		DPRINTF("COBRA :::: Map Paths: ");
		sprintf((char*)tmp, "%s\n", on?(YES):(NO));
		DPRINTF((char*)tmp);

		load_boot_plugins();
		// if (cellFsOpen("/dev_hdd0/hash_check.txt", CELL_FS_O_RDONLY, &fd, 0, NULL, 0) == 0) update_hashes();
...
/* 		if (strcmp(mount_point, "/dev_usb000") != 0 && strcmp(filesystem, "CELL_FS_FAT") != 0)
		{
			try_mount_usb0();
			if (cellFsStat("/dev_usb000/flags/nostage", &stat) == 0) {
				cellFsRename(CB_LOCATION_CEX, CB_BKP_CEX);
				cellFsRename(CB_LOCATION_DEX, CB_BKP_DEX);
				cellFsRename(SA_LOCATION, SP_LOCATION);
				cellFsRename(SM_LOCATION, SA_LOCATION);
				fatal("COBRA :::: stage2 disabled. Bye Bye...\n");
			}
		} */
...
static INLINE void load_ps2emu_stage2(int emu_type)
{
	...
	if (i == 10)
	{
		DPRINTF("COBRA :::: Failed to open debug ps2 stage2.\n");		
	}
...
static INLINE void load_ps2emu_stage2(int emu_type)
{
	...
	{
		DPRINTF("COBRA :::: Failed to open ps2 stage2: %s\n", name);
	}
...
/* static void build_netemu_params(uint8_t *ps2_soft, uint8_t *ps2_net)
{
	...
	{
		DPRINTF("COBRA :::: Cannot open ps2bootparam.dat\n");
		return;
	}
...
{	
	...
	{
		// #ifdef DEBUG
		DPRINTF("COBRA :::: Reboot into ps2_netemu LPAR (0x8204), ps2emu_type = %i\n", ps2emu_type);
		// #endif
		...
		if (ps2emu_type == PS2EMU_SW)
		{
			...
			else
			{
				...
				{
					DPRINTF("COBRA :::: NPDRM game, skipping ps2emu preparation\n");
				}
			}
		}
	}
...
LV2_PATCHED_FUNCTION(int, fsloop_open, (const char *path, int flags, int *fd, int mode, void *arg, uint64_t size))
{
	...
	if (ret == 0)
	{
		...
		{
			DPRINTF("COBRA :::: Encrypted image open: %s\n", path);
...
LV2_PATCHED_FUNCTION(int, fsloop_close, (int fd))
{
	...
	{
		DPRINTF("COBRA :::: encrypted image close\n");
...
LV2_PATCHED_FUNCTION(int, fsloop_read, (int fd, void *buf, uint64_t nbytes, uint64_t *nread))
{
	...
	{	
		if (pos&7 || nbytes&7)
		{
			DPRINTF("COBRA :::: CRITICAL: we didn't expect this kind of read %lx %lx\n", pos, nbytes);
			while (1);
		}
...
int sys_storage_ext_get_emu_state(sys_emu_state_t *state)
{
	...
	{
		DPRINTF("COBRA :::: Unknown structure size: %d, expected %ld\n", state->size, sizeof(sys_emu_state_t));
		return EINVAL;
	}
...
int sys_storage_ext_mount_discfile_proxy(sys_event_port_t result_port, sys_event_queue_t command_queue, int emu_type, uint64_t disc_size_bytes, uint32_t read_size, unsigned int trackscount, ScsiTrackDescriptor *tracks)
{
	...
	if(cd_sector_size != 2352 && cd_sector_size != 2048 && cd_sector_size != 2336 && cd_sector_size != 2448) cd_sector_size = 2352;
	trackscount &= 0xff;
        //DPRINTF("COBRA :::: CD Sector size: %i\n", cd_sector_size);
        //DPRINTF("COBRA :::: Track count: %i\n", trackscount);
	// --
	...
	if (ret == 0)
	{
		...
		if (ret == 0)
		{
			...
			if (ret == 0)
			{
				...
				if (ret != 0)
				{
					DPRINTF("COBRA :::: Failed in connecting proxy result port/queue: %x\n", ret);
					event_port_disconnect(proxy_command_port);
				}
			}
			else
			{
				DPRINTF("COBRA :::: Failed in connecting proxy command port/queue: %x\n", ret);
			}
	...
	{
		DPRINTF("COBRA :::: Cannot open even port %x (ret=%x)\n", result_port, ret);
	}
...
int sys_storage_ext_mount_encrypted_image(char *image, char *mount_point, char *filesystem, uint64_t nonce)
{
	...
	*(uint32_t *)&loop_device[0x44] = 0;
	...
	if (ret != 0)
	{
		DPRINTF("COBRA :::: cellFsUtilMount failed: %x\n", ret);
		return ret;
	}
...
static INLINE void patch_ps2emu_entry(int ps2emu_type)
{
	...
	{		
		...
		{
			DPRINTF("COBRA :::: PS2 auth patch at HV:%lx\n", search_addr+0x10);
		...
		{
			DPRINTF("COBRA :::: PS2 unauth patch at HV:%lx\n", search_addr+0x10);
			lv1_pokew(search_addr+0x10, LI(R3, 0x29));
