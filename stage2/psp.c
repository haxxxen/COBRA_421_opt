...
SprxPatch decr_psp_drm_patches[] =
{
...
int sys_psp_read_header(int fd, char *buf, uint64_t nbytes, uint64_t *nread)
{
	...
	DPRINTF("COBRA :::: umd read header: %p %lx\n", buf, nbytes);
...
int sys_psp_read_umd(int unk, void *buf, uint64_t sector, uint64_t ofs, uint64_t size)
{
	...
	DPRINTF("COBRA :::: umd read %lx %lx %lx\n", sector, ofs, size);
	...
	if (!mutex)
	{
		...
		if (ret != 0)
		{
			DPRINTF("COBRA :::: Cannot open user mutex, using an own one\n");
			mutex_create(&mutex, SYNC_PRIORITY, SYNC_NOT_RECURSIVE);
			user_mutex = 0;
		}
		else
		{
			DPRINTF("COBRA :::: user mutex opened succesfully\n");
			user_mutex = 1;
...
int sys_psp_set_umdfile(char *file, char *id, int prometheus)
{
	...
	if (prometheus)
	{
		DPRINTF("COBRA :::: Using prometheus patch.\n");
	}
#endif
	if (!patches_backup)
	{
		...
		{
			case VSH_HASH:
				DPRINTF("COBRA :::: Now patching PSP DRM into spoofed DEBUG VSH..\n");	
			...
			case VSH_DECR_HASH:
				DPRINTF("COBRA :::: Now patching PSP DRM into spoofed TOOL VSH..\n");	

				patches_backup = alloc(sizeof(decr_psp_drm_patches), 0x27);

				memcpy(patches_backup, &decr_psp_drm_patches, sizeof(decr_psp_drm_patches));

				for (int i = 0; decr_psp_drm_patches[i].offset != 0; i++)
				{
					DPRINTF("COBRA :::: Offset: 0x%08X | Data: 0x%08X\n", (uint32_t)decr_psp_drm_patches[i].offset, (uint32_t)decr_psp_drm_patches[i].data);

					copy_from_process(vsh_process, (void *)(uint64_t)(0x10000+patches_backup[i].offset), &patches_backup[i].data, 4);

					if (copy_to_process(vsh_process, &decr_psp_drm_patches[i].data, (void *)(uint64_t)(0x10000+decr_psp_drm_patches[i].offset), 4) != 0)
					{
						fatal("copy_to_process failed, you forgot to make spoofed TOOL VSH text writable, retard!\n");
					}
				}
			break;

			case VSH_NRM_HASH:
				DPRINTF("COBRA :::: Now patching PSP DRM into normal DEBUG VSH..\n");	
			...
			case VSH_CEX_HASH:
				DPRINTF("COBRA :::: Now patching PSP DRM into spoofed RETAIL VSH..\n");	
...
int sys_psp_prx_patch(uint32_t *unk, uint8_t *elf_buf, void *link_register)
{
	...
	{		
		if (*(uint32_t *)&elf_buf[0] == 0x7F454C46)
		{
			...
			DPRINTF("COBRA :::: Module %s (elf_buf=%p)\n", modinfo->modname, elf_buf);			
		}		
	}
...
/* int sys_psp_set_emu_path(char *path)
{
	...
	DPRINTF("COBRA :::: sys_psp_set_emu_path has been deleted\n");
	
	//DPRINTF("COBRA :::: pspemu path set to %s\n", path);
...
{
	DPRINTF("COBRA :::: Savedata init start\n");
...
{
	DPRINTF("COBRA :::: Savedata shutdown start\n");

	if (savedata_param)
	{
		DPRINTF("COBRA :::: Original bind: %08X\n", savedata_param[0x34/4]);
